<template>
  ...
  <div>
    <el-card shadow="hover" class="cardClass">
      <list-table :tableData="list" routerPath="adminFirst"></list-table>
      <!-- 分页 -->
...
export default {
  ...
  methods:{
    getDataList(){
      let getMethod=request.get("approve/getAdminRecheckList");
      let adminName=this.$storage.get('name');
      let currentPage=this.currentPage,pageSize=this.pageSize;
      getMethod({adminName,currentPage,pageSize}).then(({data})=>{
        this.list=data.list;
