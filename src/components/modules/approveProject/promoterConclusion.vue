<template>
  ...
  <el-container>
      ...
      <table border="1" class="test">
        ...
        <tr>
          <td colspan="6"><el-container>
            ...
            <b>项目控员成果:</b><input v-model="project.projectResult" style="width:60px;height:18px;" @blur="validProjectResult">
    ...
    <el-card class="suggestion">
      ...
      <div>
        <el-button type="primary" @click="handlePass" :disabled="validResult">结题</el-button>
        <br/><br/>
...
export default {
  ...
  methods:{
    ...
    //通过
    handlePass(){
      let adminPass=request.post("approve/promoterConclusion");
// let account=this.$storage.get('account');
      let comment=this.comment;
      let projectInfo=this.project;
      adminPass({projectInfo,comment}).then(({data})=>{
        if(data&&data.code===200){
          this.$message.success("结题成功");
          this.$router.push({path:'/conclusionPage'});
        }
      }).catch(error=>{this.$message.error("系统异常")});
    },
    validProjectResult(){
      let result=this.project.projectResult;
      let number=Number.parseFloat(result);
      if (number && number!=NaN) {
        this.project.projectResult=number;
        this.validResult=false;
      }else{
        this.project.projectResult=0;
      }
    }
  }
